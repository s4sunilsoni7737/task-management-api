...
import { LabelDocument, LabelEntity } from '../entities/label.entity';
...
export class LabelsService {
  constructor(
    @InjectModel(LabelEntity.name) private readonly labelModel: Model<LabelDocument>,
  ...
  async create(dto: CreateLabelDto, userId: string) {
    try {
      ...
      if (exists) throw new ConflictException('A label with this name already exists in this workspace');

      return await this.labelModel.create(dto);
    } catch (error) {
      if (error instanceof HttpException) throw error;
      if (error.code === 11000) throw new ConflictException('A label with this name already exists');
      throw new BadRequestException({
