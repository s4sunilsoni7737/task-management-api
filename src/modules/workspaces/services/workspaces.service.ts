...
import { WorkspaceDocument, WorkspaceEntity } from '../entities/workspace.entity';
...
export class WorkspacesService {
  ...
  async getOne(id: string, userId: string) {
    const workspace = await this.workspaceModel.findOne({ _id: id, isDeleted: false }).select('-__v').lean();
  ...
  async assertUserIsMember(workspaceId: string, userId: string) {
    const workspace = await this.workspaceModel.findOne({ _id: workspaceId, isDeleted: false }).lean();
    if (!workspace) throw new NotFoundException('Workspace not found');
