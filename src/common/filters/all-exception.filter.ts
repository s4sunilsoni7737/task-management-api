...
export class AllExceptionsFilter implements ExceptionFilter {
  ...
  async catch(exception: unknown, host: ArgumentsHost): Promise<any> {
    ...
    if (exception instanceof HttpException) {
      ...
      let developerMessage = exceptionResponse?.developerMessage || exceptionResponse?.message || 'Something went wrong'
      ...
      formattedResponse = {
        ...
        developerMessage: typeof developerMessage === 'string' ? developerMessage : developerMessage.toString(),
        data: {},
      }
    }
    else {
    ...
    httpAdapter.reply(ctx.getResponse(), { logId: logDoc?._id, ...formattedResponse }, formattedResponse['statusCode'])
  }
}